def wrap_in_css(source, title):
    ...
    """ Adds the CSS header and footer to the page source and returns the result in a easier to read format with plenty of whitespace.
    """

    source = ''.join(source)

    with open("./header.txt") as header_filename:
        header = header_filename.read()
        header = header.replace('<title>!!!DYNAMIC_TITLE!!!', '<title>{0}'.format(title))
        source = '{0}{1}'.format(header, source)

    with open("./footer.txt") as footer_filename:
